<template>
  <Popover trigger='click' v-if="input || popup" v-bind="popoverProps">
    <template #content>
      <component :style="innerPickerStyles" :is="Picker" v-model="innerState" @change="triggerOnChange"
        @changeComplete="triggerOnChangeComplete" v-bind="innerPickerProps" />
    </template>
    <InputGroup v-if="input" readonly compact style="display: flex;" v-bind:style="$attrs.style">
      <Input :value="readAbleModelValue" readonly>
      <template #addonAfter>
        <div :style="blStyles"></div>
      </template>
      </Input>
    </InputGroup>
    <div v-else :style="blStyles"></div>
  </Popover>


  <component :style="innerPickerStyles" :is="Picker" v-else v-model="innerState" @change="triggerOnChange"
    @changeComplete="triggerOnChangeComplete" v-bind="innerPickerProps" />
</template>
<script lang="ts" setup name="AColorPicker">
import { computed, ref } from 'vue';
import type { Color, ColorPickerProps, ColorResult } from './types'
...
const props = defineProps({
  valueFormat: PropTypes.oneOf(['hex', 'hex8', 'rgba', 'rgb']),
...
const innerState = computed({
  ...
  set: (_value) => {
    ...
    emit('update:modelValue', getEmitV())
  }
})
